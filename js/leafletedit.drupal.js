...
(function() {
  ...
  $(document).one("leafletMapInit", function (e, initial, self) {
    ...
    /// Load layers
    panel = L.control.panelLayers(null, null, {collapsed: true, compact: true, collapsibleGroups: true });
    map.bounds=null;
 ...
 */
      ...
      .on('data:loaded', function () {
          for(k in this._layers) {
            /* if (this._layers[k].feature.style) {
              if (this._layers[k].feature.style['fill']) {
            ...
            if (this._layers[k].feature.properties) {
              for(k1 in this._layers[k].feature.properties) {
                switch (k1) {
                  case 'boucle_man':
                    if (this._layers[k].feature.properties[k1] == 'AM') {
                      boucle='Aprem';
                    }
                    else {
                      boucle='Matin'
                    ...
                    break;
                  case 'desc':
                    if (this._layers[k].feature.properties[k1]) {
                      description=this._layers[k].feature.properties[k1];
                    }
                    break;
                  case 'nom_manif':
                    manif=this._layers[k].feature.properties[k1]
                    break;
                  case 'type':
                    switch(this._layers[k].feature.properties[k1]) {
                      case 'o':
                        type='Option';
                      break;
                      case 'p':
              ...
              colors = scm.colors().filter(function(e) { return e !== 'ffffff' })
              switch(type) {
                case 'Option':
                  color=colors[0];
                break;
              ...
              this._layers[k].setStyle({'color':'#'+color, 'weight':5});
              this._layers[k].bindTooltip(manif + ' ' + boucle + '-' + type, {'sticky': true} );
